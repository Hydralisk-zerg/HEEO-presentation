...
const StartPage = ({ onLanguageSelect, onPresentationStart, t, currentLanguage }) => {
  ...
  // Язык-переключатель (показываем противоположный текущему)
  const toggleLang = currentLanguage === 'en' ? 'uk' : 'en';
  const toggleFlag = toggleLang === 'uk' ? '🇺🇦' : '🇺🇸';
  const toggleLabel = t.languages[toggleLang];
    ...
    >
      ...
      >
        ...
        <Row gutter={[styles.gutter, styles.gutter]} style={{ width: '100%' }}>
          {/* Выбор языка */}
          <Col xs={24} md={8} style={{ textAlign: 'center', display: 'flex' }}>
            <Card
              hoverable
              style={{
                textAlign: 'center',
            ...
            >
              ...
              <div style={{ width: '100%' }}>
                <Row gutter={[0, 0]} style={{ width: '100%' }}>
                  <Col span={24}>
                    <Button
                      ...
                      onClick={() => onLanguageSelect(toggleLang)}
                      icon={
                        <span style={{ 
                ...
                {/* Отступ для выравнивания с другими карточками */}
